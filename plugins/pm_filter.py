...
async def next_page(bot, query):
    ...
    if settings['button']:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {file.file_name}", url=await get_shortlink(f" https//:telegram.me/{temp.U_NAME}?start=files_{files.files_id}")
                ),
            ]
            for file in files
        ]
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"{file.file_name}", url=await get_shortlink(f" https//:telegram.me/{temp.U_NAME}?start=files_{files.files_id}")
                ),
                InlineKeyboardButton(
                    text=f"{get_size(file.file_size)}",
                    url=await get_shortlink(f" https//:telegram.me/{temp.U_NAME}?start=files_{files.files_id}")
                ),
            ]
            for file in files
        ]
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {file.file_name}", url=await get_shortlink(f" https//:telegram.me/{temp.U_NAME}?start=pre_{files.files_id}")
                ),
            ]
            for file in files
        ]
        ...
        btn = [
            [
                InlineKeyboardButton(
                    ...
                    url=await get_shortlink(f" https//:telegram.me/{temp.U_NAME}?start=pre_{files.files_id}")
                ),
                InlineKeyboardButton(
                    text=f"{get_size(file.file_size)}",
                    url=await get_shortlink(f" https//:telegram.me/{temp.U_NAME}?start=pre_{files.files_id}")
